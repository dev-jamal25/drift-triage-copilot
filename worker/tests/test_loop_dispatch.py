"""Loop ↔ dispatcher integration — Step 4 + 5.
...
async def test_replay_test_action_acks(adapter: AsyncMock, retry_policy: RetryPolicy) -> None:
    raw = _payload("replay_test")

    await process_message(adapter, raw, dispatch_fn=dispatch, retry_policy=retry_policy)
...
async def test_retrain_action_acks(adapter: AsyncMock, retry_policy: RetryPolicy) -> None:
